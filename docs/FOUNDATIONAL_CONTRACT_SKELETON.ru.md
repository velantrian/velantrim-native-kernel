...
- **Версия контракта:** `foundational-skeleton/1.0`
...
> [!IMPORTANT]
> Этот документ описывает принятый архитектурный скелет, а не готовую работающую машину. Принятие фиксирует контрактную границу и стабильные пространства assertion ID. Оно не доказывает поведение runtime, executable conformance, переносимость между технологиями или production-ready состояние.
...
Принятый каркас делит фундамент на шесть семейств контрактов:
...
Эти роли МОГУТ выражаться через `claim_kind`, типизированные связи, mapping профиля или другой версионируемый контракт. В этом контракте они не являются обязательным enum.

## 4.3 Контрактные assertions
...
Это семантические слои. Они не являются расширением принятого исследовательского event vocabulary.
...
Conflict Set — семантический паттерн, а не обязательная корневая сущность или event vocabulary.
...
Этот контракт не делает постоянной архитектурой:
...
| Этот six-family skeleton | ADR-0010 — `ACCEPTED` |
...
1. Сохранять принятие ADR-0010 и aliases версии `foundational-skeleton/1.0` при будущих изменениях.
