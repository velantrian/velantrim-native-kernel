...
- `native_kernel.postgresql_profile.replay` — replay, projection persistence, destroy/rebuild and stale-head guard;
- `native_kernel.postgresql_profile.verified_replay` — linked rebuild-Receipt consistency verification;
...
```text
8afcb931b5da521192ffb379682f08d1ba62df3d
```

Workflow results:

```text
P3 replay/projection run 31172243736 — PASS
P2 regression run         31172243692 — PASS
P1 semantic core run      31172243709 — PASS
Fixture integrity run     31172243710 — PASS
AI context run            31172243728 — PASS
```
...
- 8 PostgreSQL integration tests, including valid-but-mismatched projection Receipt rejection;
- 9 P2 unit tests;
- 5 P2 PostgreSQL integration tests;
- Python compileall.

The PR final documentation head must repeat affected checks before merge. Evidence applies only to the exact SHA/run named above.
...
4. A duplicate tracker Issue #51 was created while resuming context; it was closed as a duplicate of canonical Issue #49.
...
- projections are disposable but their operational monitoring/repair policy is absent;
