...
### Possible future complementarity (research-stage, non-binding)

> Both projects are unfinished and independently developed. The points below describe *where their current research already overlaps*, not an approved integration. They do not lift any boundary stated above.

- **Replay determinism.** Native Kernel's [`ADR-0002`](./adr/0002-state-checkpoints-are-disposable.md) (disposable State Checkpoints) and [`ADR-0004`](./adr/0004-rebuild-from-authoritative-history.md) (rebuild-from-history conformance) are proposing the same abstract shape that Mentaury Soul has already implemented and tested end-to-end in its R1 work (`docs/P0_013_R1_DETERMINISTIC_REPLAY.md`: `state_hash(full replay) == state_hash(verified snapshot + tail replay)`). Mentaury's SQLite implementation is a concrete, already-tested data point Native Kernel could study — without adopting the code — before finalizing its own equivalence rule and Issue #1 conformance experiment.
- **Redaction of immutable history.** This document already names "redaction-aware history" as a future Native Kernel substrate primitive, but it is undesigned. Mentaury Soul's P0-010 (`docs/P0_010_ATOMIC_SAME_STREAM_REDACTION.md`) is a working, governed, same-stream, byte-for-byte-immutable-row redaction mechanism. It is a candidate reference point for the eventual Native Kernel redaction ADR.
- **Evidence-gated state transitions.** Native Kernel's Receipts and its proposed Audit Curiosity profile describe a deterministic-gate pattern for turning evidence into an accepted/rejected state change. Mentaury Soul's Evidence Gate (`docs/P0_015_EVIDENCE_GATE.md`) already implements a structurally similar gate for belief status. The two could eventually share vocabulary for what counts as a valid receipt, without sharing runtime.
- **Typed relationships.** [`ADR-0006`](./adr/0006-causal-links-are-relations.md) proposes typed, directed relations as a substrate-neutral contract, deliberately separate from lineage. Mentaury Soul studies relationships and commitments as first-class objects and is an independent, real-world consumer this contract could eventually be checked against — if that contract matures and the governed-promotion sequence above is followed.

None of the above authorizes shared code, shared schemas, or a dependency between the two repositories today. It only records that two independently developed research tracks are, in places, studying the same problem from different sides, and could inform each other's design discussions before either one changes runtime behavior.

Formalized on each side as citation-only research-input practice, without lifting any boundary above: [`ADR-0010`](./adr/0010-mentaury-soul-implementations-as-external-research-input.md) and [`velantrim-mentaury-soul` docs/research/NATIVE_KERNEL_RESEARCH_INPUT_NOTES_V0.1.md](https://github.com/velantrian/velantrim-mentaury-soul/blob/main/docs/research/NATIVE_KERNEL_RESEARCH_INPUT_NOTES_V0.1.md).
