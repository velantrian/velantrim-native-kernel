...
| [`GLOSSARY.ru.md`](./GLOSSARY.ru.md) | onboarding terminology; provisional A2–A7 blueprint distinctions имеют приоритет during integrated review |
...
→ provisional blueprint deliverables A1–A7
...
A4 law-set candidate — `nk-semantic-laws/A4-draft-1` с 28 provisional laws. A5 candidate — `nk-identity-time-change/A5-draft-1`, defining typed/scoped identity, named temporal/order relations и explicit change effects без требования одного physical encoding. A6 candidate — `nk-knowledge-lifecycle/A6-draft-1`, моделирующий knowledge lifecycle как labeled directed graph девяти recurring phases. A7 candidate — `nk-conflict-uncertainty-revision/A7-draft-1`, разделяющий tension kind, assessment status, resolution status, typed uncertainty, Authority, scoped resolution/reopening и explicit revision без universal winner algorithm или confidence scalar.
